...
const DynamicInvoice = dynamic(() =>
  import('@features/client').then(({ Invoice }) => Invoice)
);
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) => async (ctx) => {
    ...
    await store.dispatch(
      getSpecialistsProfileThunkSSR({ id, areaType: query.area as TAreaType })
    );
...
ClientInvoice.getLayout = (page: ReactNode) => <CommonPage>{page}</CommonPage>;
